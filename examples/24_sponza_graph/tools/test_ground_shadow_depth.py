...
aspect = 1280 / 720
near_z, far_z = 0.4, 50.0
f = 1.0 / math.tan(60.0 * 0.5 * math.pi / 180.0)
proj = (f / aspect, f)

dir_light = (0.35, -0.90, 0.25)
...
# ground in cast shadow (opposite light travel) and cube surface point
test_pts = [
    ("ground_shadow", (-1.4, 0.01, -0.9)),
    ("ground_lit", (2.5, 0.01, 2.0)),
    ("cube_top", (0.0, 1.5, 0.0)),
    ("cube_sun_face", (0.5, 0.75, 0.0)),
]
...
for ci in range(4):
    ...
    for name, p in test_pts:
        ...
        depths[name] = ndc[2]
        print(f"  {name} p={p} ndc.z={ndc[2]:.4f}")
    gs = depths["ground_shadow"]
    ct = depths["cube_top"]
    print(f"  ground_shadow vs cube_top: ground should be BEHIND (larger z) -> shadow if gs>{ct}: {gs > ct}")
