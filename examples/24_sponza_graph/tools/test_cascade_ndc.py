#!/usr/bin/env python3
"""CPU replay of computeOrthographicCascades; check if scene points land in NDC."""
import math
import struct

bounds_min = (-1920.945923, -126.442665, -1182.806885)
bounds_max = (1799.908081, 1429.43335, 1105.42627)

cam_pos = (321.0, 733.0, -40.0)
yaw, pitch = -1.62, 0.0
...
aspect = 1280 / 720
fov = 60.0
near_z, far_z = 3.0, 6000.0
f = 1.0 / math.tan(fov * 0.5 * math.pi / 180.0)
proj = (f / aspect, f)

dir_light = (0.55, -0.78, 0.28)
...
for ci in range(4):
    ...
    for p in test_pts:
        ...
        if inside:
            in_count += 1
        print(f"  cascade {ci} p={p} ndc=({ndc[0]:.3f},{ndc[1]:.3f},{ndc[2]:.3f}) in={inside}")
    print(f"cascade {ci} splitFar={sp[ci+1]:.1f} in_ndc={in_count}/{len(test_pts)}")
